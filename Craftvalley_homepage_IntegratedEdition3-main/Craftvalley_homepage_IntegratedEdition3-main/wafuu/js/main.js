// ================================================================
// CRAFTVALLEY — 和風 VERSION — MAIN JS (sidebar nav variant)
// ================================================================
...
(function () {
  ...
  function setLang(lang) {
    ...
    document.dispatchEvent(new Event("cv-lang-change"));
  }
  ...
  function updateLangToggle() {
    document.querySelectorAll(".sidebar-lang, .mobile-lang").forEach(function (btn) {
  ...
  function initNav() {
    ...
    // Inject an always-visible mobile language toggle (mirrors the hamburger).
    // It carries class "mobile-lang", so initLangToggle()/updateLangToggle() wire it automatically.
    if (!document.querySelector(".mobile-lang")) {
      var mLang = document.createElement("button");
      mLang.className = "mobile-lang";
      mLang.setAttribute("aria-label", "Switch language");
      mLang.textContent = window.CV_LANG === "en" ? "日本語" : "English";
      document.body.appendChild(mLang);
    }

    // Add a small "LANGUAGE / 言語" hint above the desktop sidebar toggle.
    var sbBottom = document.querySelector(".sidebar-bottom");
    if (sbBottom && !sbBottom.querySelector(".sidebar-lang-label")) {
      var label = document.createElement("span");
      label.className = "sidebar-lang-label";
      label.textContent = "Language / 言語";
      sbBottom.insertBefore(label, sbBottom.firstChild);
    }

    // Inject a backdrop behind the slide-in sidebar (mobile).
    ...
    // Active nav link — detail/sub pages highlight their parent section
    var PARENT_PAGE = {
      "producer-detail.html": "producers.html",
      "product-detail.html": "products.html",
      "events-past.html": "events-upcoming.html",
      "calendar.html": "events-upcoming.html",
    };
    var currentPage = window.location.pathname.split("/").pop() || "index.html";
    var navTarget = PARENT_PAGE[currentPage] || currentPage;
  ...
  function initCalendar() {
    ...
    function render() {
      ...
      grid.innerHTML = "";
      (lang === "ja" ? dayNamesJa : dayNames).forEach(function (d) {
          ...
          (function (ev) {
            cell.addEventListener("click", function () {
              ...
              popup.innerHTML = ev.map(function (e) {
                return '<p style="margin-bottom:8px"><strong style="color:var(--kin2)">' + e.date + '</strong> — ' + e.title + '</p>';
              }).join("");
            });
          })(eventsOnDay);
        }
    ...
    calWrap.querySelector(".cal-prev").addEventListener("click", function () {
      current.month--; if (current.month < 0) { current.month = 11; current.year--; } render();
    });
    calWrap.querySelector(".cal-next").addEventListener("click", function () {
      current.month++; if (current.month > 11) { current.month = 0; current.year++; } render();
    });
    render();
  ...
  function populateProducers() {
    ...
    CV_DATA.producers.forEach(function (p, idx) {
      ...
      card.innerHTML =
        '<div class="producer-card-img"><div class="image-placeholder"><span class="image-placeholder-label">🏔️</span>' +
        ...
        '</div>' +
        '<div class="producer-card-footer"><span class="producer-card-link" data-i18n="producers.viewProfile">' + getT("producers.viewProfile") + '</span></div>';
  ...
  function populateEventsPreview() {
    ...
    function renderEventItem(e, isPast) {
      ...
      item.innerHTML =
        '<div class="event-date-box"><div class="event-month">' + (lang === "ja" ? monthsJa[d.getMonth()] : months[d.getMonth()]) + '</div>' +
        '<div class="event-day">' + d.getDate() + '</div><div class="event-year">' + d.getFullYear() + '</div></div>' +
        '<div class="event-info"><p class="event-type">' + e.type + '</p>' +
        '<h3 class="event-title">' + (lang === "ja" ? e.titleJa : e.titleEn) + '</h3>' +
        '<p class="event-location">' + (lang === "ja" ? e.locationJa : e.locationEn) + '</p></div>' +
        '<span class="event-cta">' + (isPast ? (lang === "ja" ? "詳細" : "Details") : (lang === "ja" ? "詳細 →" : "Details →")) + '</span>';
      return item;
    }
    CV_DATA.events.upcoming.slice(0, 3).forEach(function (e) { upcomingList.appendChild(renderEventItem(e, false)); });
    if (pastList) CV_DATA.events.past.slice(0, 3).forEach(function (e) { pastList.appendChild(renderEventItem(e, true)); });
    initScrollAnimations();
  }
  ...
  function populateNewsPreview() {
    ...
    CV_DATA.news.slice(0, 4).forEach(function (n, idx) {
      ...
      card.innerHTML =
        ...
        '<div class="news-card-body"><p class="news-cat">' + n.category + '</p>' +
        '<h3 class="news-title">' + (lang === "ja" ? n.titleJa : n.titleEn) + '</h3>' +
        '<p class="news-date">' + n.date + '</p></div>';
      grid.appendChild(card);
    });
    initScrollAnimations();
  }
  ...
  function populateProducerDetail() {
    ...
    wrap.innerHTML =
      '<div class="producer-detail-grid">' +
        '<div class="producer-detail-img"><div class="image-placeholder"><span class="image-placeholder-label">🏔️</span>' +
        '<div class="image-placeholder-note">' + producer.image + '</div></div></div>' +
        '<div class="producer-detail-info">' +
          ...
          '<p style="color:var(--textm);font-size:16px;font-style:italic;margin-bottom:20px">' + (lang === "ja" ? producer.nameEn : producer.nameJa) + '</p>' +
          '<p style="color:var(--textm);font-size:15px;line-height:1.95;margin-bottom:24px">' + (lang === "ja" ? producer.bioJa : producer.bioEn) + '</p>' +
          '<div class="producer-meta">' +
            '<div class="meta-item"><p class="meta-label">Established</p><p class="meta-value">' + producer.established + '</p></div>' +
            '<div class="meta-item"><p class="meta-label">Generation</p><p class="meta-value">' + producer.generation + (lang === "ja" ? "代目" : "th Gen") + '</p></div>' +
          ...
          '</div>' +
          '<div class="access-box"><h4>' + getT("producers.access") + '</h4><p>' + (lang === "ja" ? producer.accessJa : producer.accessEn) + '</p></div>' +
        '</div></div>' +
      '<h2 class="section-heading fade-in" style="margin-bottom:32px;margin-top:40px">' + getT("producers.products") + '</h2>' +
    ...
    relatedProducts.forEach(function (p) {
      ...
      card.innerHTML = '<div class="product-card-img"><div class="image-placeholder"><span class="image-placeholder-label">🎁</span><div class="image-placeholder-note">' + p.image + '</div></div></div><div class="product-card-body"><p class="product-name">' + (lang === "ja" ? p.nameJa : p.nameEn) + '</p><p class="product-price">$' + p.price + '</p></div>';
    ...
    wrap.innerHTML =
      '<div class="product-detail-grid">' +
        ...
        '<div class="product-detail-info">' +
          ...
          (producer ? '<p style="font-size:14px;color:var(--textm)">By <a href="producer-detail.html?id=' + producer.id + '" style="color:var(--aka)">' + (lang === "ja" ? producer.nameJa : producer.nameEn) + '</a></p>' : '') +
          '<div class="product-specs">' +
            ...
            '<div class="spec-row"><span class="spec-label">Stock</span><span>' + (product.inStock ? (lang === "ja" ? "在庫あり" : "In Stock") : "Out of Stock") + '</span></div>' +
          '</div>' +
  ...
  function populateFullEvents(type) {
    ...
    events.forEach(function (e) {
      ...
      if (type === "upcoming") {
        ...
        card.innerHTML =
          '<div class="event-card-date"><div class="month">' + (lang === "ja" ? monthsJa[d.getMonth()] : months[d.getMonth()]) + '</div><div class="day">' + d.getDate() + '</div><div class="year">' + d.getFullYear() + '</div></div>' +
          '<div class="event-card-info"><span class="event-type-badge">' + e.type + '</span><h2 class="event-card-title">' + (lang === "ja" ? e.titleJa : e.titleEn) + '</h2><p class="event-card-location">📍 ' + (lang === "ja" ? e.locationJa : e.locationEn) + '</p><p class="event-card-desc">' + (lang === "ja" ? e.descJa : e.descEn) + '</p></div>' +
        ...
        card2.innerHTML =
          '<div class="event-card-date"><div class="month">' + (lang === "ja" ? monthsJa[d.getMonth()] : months[d.getMonth()]) + '</div><div class="day" style="font-size:28px">' + d.getDate() + '</div><div class="year">' + d.getFullYear() + '</div></div>' +
          '<div><h3 style="font-size:17px;margin-bottom:6px;font-family:var(--ff-ja)">' + (lang === "ja" ? e.titleJa : e.titleEn) + '</h3><p style="font-size:13px;color:var(--textm)">📍 ' + (lang === "ja" ? e.locationJa : e.locationEn) + '</p></div>' +
  ...
  function populateNewsPage() {
    ...
    CV_DATA.news.forEach(function (n) {
      ...
      card.innerHTML =
        '<div class="news-full-img"><div class="image-placeholder"><span class="image-placeholder-label">📰</span><div class="image-placeholder-note">' + n.image + '</div></div></div>' +
        '<div class="news-full-body"><p class="news-cat">' + n.category + '</p><h2 class="news-title" style="font-size:20px;margin-bottom:10px">' + (lang === "ja" ? n.titleJa : n.titleEn) + '</h2><p style="font-size:14px;color:var(--textm);line-height:1.8;margin-bottom:10px">' + (lang === "ja" ? n.summaryJa : n.summaryEn) + '</p><p class="news-date">' + n.date + '</p><span class="news-read-more">' + getT("news.readMore") + ' →</span></div>';
      grid.appendChild(card);
    });
    initScrollAnimations();
  }

  // Populate all data-driven sections (also re-run on language change)
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    initNewsletterForm();
    populateAll();
    document.addEventListener("cv-lang-change", populateAll);
